...
--밑수와 지수를 각각 물어보게 하고 계산된 지수 함수의 결과가 출력되게 하는 PL/SQL을 작성하시오.
...
--밑수와 진수를 각각 물어보게 하고 계산된 로그 함수의 결과가 출력되게 하는 PL/SQL문을 작성하시오.
...
--158. 기초통계 구현하기 1 (평균값)
--여러 개의 숫자를 입력받은 후 입력 받은 숫자들의 평균 값을 출력하는 PL/SQL문을 작성하시오.
...
-- 키와 체중 데이터를 각각 입력받은 후 키와 체중 간의 상관관계가 있는지 PL/SQL문을 구현해보시오.
...
--164.  기초 통계 구현하기 7 (확률)
-- 하나의 동전을 여러 번 던졌을 때 앞면과 뒷면이 나올 확률이 각각 50%임을 PL/SQL문로 확인해보시오.
...
--165. 기초 통계 구현하기 8 (확률)
-- 동전 두개를 동시에 던져 둘 다 앞면이 나올 확률과 
-- 둘 다 뒷면이 나올 확률, 
-- 하나는 앞면 하나는 뒷면일 나올 확률을 각각 출력해보시오.
...
begin
   ...
   dbms_output.put_line('동전 둘중 하나가 앞면, 다른 하나는 뒷면이 나올 확률  '|| 
round((v_2/v_loop),2));    
...
-- 초등학생 십만 명의 키 숫자 데이터를 랜덤으로 생성하여 
-- 십만 명의 키 데이터가 정규분포를 이루는지 PL/SQL로 구현해보시오.
...
-- 가로의 길이와 세로의길이, 빗변의 길이를 각각 입력받고 직각삼격형인지의 여부를 출력하는 PL/SQL문을 작성하시오.
